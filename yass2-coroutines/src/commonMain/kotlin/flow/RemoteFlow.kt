...
public interface FlowService<out F, I> { // TODO: review
...
public fun <F, I> FlowService<F, I>.createFlow(flowId: I): Flow<F> =
    @OptIn(ExperimentalCoroutinesApi::class) // TODO: might become binary incompatible with the future versions of the standard library
        ...
        override suspend fun create(flowId: I): Int {
            ...
            tryCatch({
                ...
                CoroutineScope(coroutineContext).launch {
                    tryFinally({
                        try {
                            flow.collect { channel.send(ValueReply(it)) } // TODO: send with timeout
                            channel.send(null)// TODO: send with timeout
                        } catch (e: Exception) {
                            channel.send(ExceptionReply(e))// TODO: send with timeout
                        }
                    }) {
